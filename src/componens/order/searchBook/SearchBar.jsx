...
export const SearchBar = ({ search, posit, searchHandler }) => {
    ...
    const[searchType , setSearchType] = useState("name")
        let checkClick = (i) => {
            ...
            setCheck(arrayColor)
            if (i == 0 ) setSearchType("name")
            if (i == 1 ) setSearchType("major")
            if (i == 2 ) setSearchType("code")
            ...
            <motion.div exit="{{ opacity: 0 }}" initial={{ ...JSON.parse(JSON.stringify(posit)), x: JSON.parse(JSON.stringify(posit)).x * 0.74545, y: JSON.parse(JSON.stringify(posit)).y * 0.4567, borderRadius: "4px" }}
                ...
                <motion.div className="CardInner" initial={{ opacity: 0 }} transition={{ delay: 1.4 }} animate={{ opacity: 1 }}>
                    ...
                    <div className="container2">
                        <div className="Icon" onClick={() => searchHandler(searchText,searchType)}>
                        ...
                        <Auto placeholder={placeholder} searchText={setSearchText} searchType={searchType}/>
                            {/* <input className="input" onChange={(e) => setSearchText(e.target.value)} placeholder={"..." + placeholder} /> */}
                        </div>
                    </div>
                </motion.div>
            </motion.div>
