    ...
    return (
        <div className="ditails">
            ...
            <div className="ditails__table">
                {names.map((ele, _j) => <div key={_j} className="ditails__row">  <h5 className="ditails__row--main">{ele.name}</h5> {books.map((book, i) => <h5 key={i} className="ditails__column" style={{ backgroundColor: i % 2 == 0 ? "" : "" }}>{!(ele.to == "status") ? book[ele.to] : <div className="ditails__last"> <img onClick={() => dispatch(deleteBooksUser(book))} src={Delete} alt="" /> <img onClick={() => history.push(`/dash/edit/${book._id}`)} src={LoopArrow} alt="" /> <img src={TickMark} alt="" /> </div>}</h5>)} </div>)}
            </div>
        </div>
    )
})
